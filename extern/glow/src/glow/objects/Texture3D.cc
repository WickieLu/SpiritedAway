...
static GLOW_THREADLOCAL Texture3D::BoundTexture3D *sCurrentTexture3D = nullptr;

Texture3D::BoundTexture3D *Texture3D::getCurrentTexture()
{
    return sCurrentTexture3D;
}
...
{
    GLOW_RUNTIME_ASSERT(sCurrentTexture3D == this, "Currently bound FBO does NOT match represented Texture " << to_string(texture), return false);
    return true;
}
...
void Texture3D::BoundTexture3D::setBorderColor(tg::color4 const& color)
{
    if (!isCurrent())
        return;
    checkValidGLOW();
...
Texture3D::BoundTexture3D::BoundTexture3D (Texture3D *texture) : texture(texture)
{
    ...
    previousTexturePtr = sCurrentTexture3D;
    sCurrentTexture3D = this;
}
...
Texture3D::BoundTexture3D::~BoundTexture3D ()
{
    if (previousTexture != -1) // if valid
    {
        ...
        sCurrentTexture3D = previousTexturePtr;
    }
}
